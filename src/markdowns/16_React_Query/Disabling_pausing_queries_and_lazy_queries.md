...
- `refetch` function returned from `useQuery` can be used to manually trigger the query to fetch.
...
If you are using disabled or lazy queries, you can use the `isLoading` flag instead. It's a derived flag that is computed from: ```isPending && isFetching``` so it will only be true if the query is currently fetching for the first time.

## References

1. [Disabling/Pausing Queries](https://tanstack.com/query/latest/docs/react/guides/disabling-queries)