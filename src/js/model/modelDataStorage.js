  ...
  sections: [
    ...
    },
    {
      ...
      tooltips: [
        ...
        <p>But as the Web became more interactive, <b>logic increasingly determined content. JavaScript was in charge of the HTML!</b> This is why <i>in React, rendering logic and markup live together in the same place — components</i>.</p>
        <p>Keeping a button's <i>rendering logic and markup together</i> ensures that they <i>stay <b>in sync</b> with each other</i> on every edit. Conversely, details that are unrelated, such as the button's markup and a sidebar's markup, are <b>isolated</b> from each other, making it safer to change either of them on their own.</p>
        `,
        `<h3>JSX: Putting markup into JavaScript</h3>
        <p>Each React component is a JavaScript function that may contain some markup that React renders into the browser. React components use a <i><b>syntax extension</b> called JSX</i> to represent that markup. JSX looks a lot like HTML, but it is a bit stricter, has a few more rules than HTML and can display dynamic information.</p>
        <p>If you have some (perfectly valid) HTML markup and you want to put it into your component by copy and paste it as is, it won't always work. This is because JSX is stricter and has a few more rules than HTML! <i>JSX markup is written like HTML, but it is actually JavaScript under the hood! JSX just provides <b>syntactic sugar</b> for the <code>React.createElement()</code> function.</i></p>
        <p><i>JSX lets you <b>write HTML-like markup inside a JavaScript file</b>, keeping rendering logic and content in the same place</i>.</p>
        `,
        `<h3>You can store JSX markup in variables</h3>
        <p><i>You are not limited to using JSX only if you return it, you can also use it to create a value, which is stored in a variable.</i></p>
        `,
    ...
    },
    {
      ...
      tooltips: [
        ...
        <p>JSX lets you write HTML-like markup inside a JavaScript file, keeping rendering logic and content in the same place. <i>Sometimes you will want to add a little JavaScript logic or reference a dynamic property inside that markup. In this situation, you can <b>use curly braces in your JSX to “open a window” to JavaScript</b>.</i></p>
        ...
        <p><i>Curly braces <code>{}</code> let you <b>bring <u>JavaScript logic</u> and <u>variables</u> into your markup</b>!</i></p>
        ...
        <p>JSX is very minimal as a templating language because it lets you organize data and logic using JavaScript.</p>
        `,
        `<h3>JSX curly braces & JSX attributes</h3>
        <p>JSX attributes inside quotes are passed as strings, but <i>with curly braces you can also "escape into JavaScript" from JSX attributes</i>.</p>
        `,
        `<h3>Using "double curlies": CSS and other objects in JSX</h3>
        <p>In addition to strings, numbers, and other JavaScript expressions, <b>you can even pass objects in JSX</b>. To pass a JavaScript object in JSX, <i>you must wrap the object in another pair of curly braces</i>.</p>
    ...
    },
    {
      ...
      tooltips: [
        ...
        `,
        `<h3>Summary</h3>
        <p>1. <b><code>npm create vite@latest</code></b></p>
        <p>2. <b><code>npm install</code></b></p>
        `,
      ],
    },
    {
      ...
      tooltips: [
        ...
        <p>React lets you combine your markup, CSS, and JavaScript into custom “components”, reusable UI elements for your app. <i>Just like with HTML tags, <b>you can compose, order and nest components to design whole pages</b>.</i> As your project grows, you will notice that many of your designs can be composed by <i>reusing components you already wrote, speeding up your development</i>.</p>
        <p>The key difference between React components and built-in HTML tags is that <i><b>React components names must always start with a capital letter</b>, while HTML tags must be lowercase</i>. React components are regular JavaScript functions, but their names must start with a capital letter or they won't work!</p>
    ...
    },
    {
      ...
      tooltips: [
        ...
        <p>React components use props to communicate with each other. <b>Every <u>parent component</u> can pass some information to its <u>child components</u> by giving them props</b>. Props might remind you of HTML attributes, but <i>you can pass any JavaScript value through props, including objects, arrays, functions, and even JSX markup!</i></p>
        ...
        <p>To pass props, add them to the JSX markup, just like you would with HTML attributes. Props let you think about parent and child components independently.</p>
        <p><i>Props serve the same role as arguments serve for functions</i> — in fact, <i>props are the only argument to your component!</i> <b>React component functions accept a single argument, a <code>props</code> object.</b></p>
        ...
        <p><i>The convention for props which hold functions is to start their name with "on".</i></p>
        `,
        ...
        <p><i>With the help of <code>children</code> prop you can build <b>wrapper components</b></i>. Wrapper components can wrap any nested content. It doesn't need to “know” what's being rendered inside of it.</p>
        ...
        <p><i>A component may receive different props over time. <b>Props are not always static!</b> Props reflect a component's data at any point in time, rather than only in the beginning.</i></p>
        <p><i>However, <b>props are immutable</b> — a term from computer science meaning “unchangeable”.</i> When a component needs to change its props (for example, in response to a user interaction or new data), it will have to “ask” its parent component to pass it different props — a new object! Its old props will then be cast aside, and eventually the JavaScript engine will reclaim the memory taken by them.</p>
        <p>Props are <i><b>read-only</b> snapshots in time: every render receives a new version of props</i>.</p>
    ...
    },
    {
      ...
      tooltips: [
        ...
        <p>Sometimes you wanna have a component where you don't configure everything through props, but where instead you're able to <i><b>pass content (JSX markup)</b> between the opening and closing tags of that component</i>.</p>
        ...
        <p>If you want to use <b>custom components as wrappers</b>, you need to use one special prop that is build into React, which every component receives, even if you're never setting it explicitly, and that is <b><code>props.children</code></b>.</p>
        <p><i>With the help of <code>props.children</code> prop you can build <b>wrapper components</b></i>. Wrapper components can wrap any nested content. It doesn't need to “know” what's being rendered inside of it. You can think of a component with a <code>props.children</code> prop as having a "hole" that can be "filled in" by its parent components with arbitrary JSX markup.</p>
        <p><code>props.children</code> is a reserved name, and <i>the value of this special <code>children</code> prop will always be the content between the opening and closing tags of your custom component</i>. This is a concept called composition.</p>
        `,
      ],
    },
    {
    ...
    },
    {
      ...
      tooltips: [
        ...
        <p><i>You can declare many components in one file</i>, but large files can get difficult to navigate. To solve this, you can export a component into its own file, and then import that component from another file.</p>
        ...
        <p>Regardless of which coding style you prefer, <b>always give meaningful names to your component functions and the files that contain them</b>. Components without names, like <code>export default () => {}</code>, are discouraged because they make debugging harder.</p>
        `,
      ],
    },
    {
      ...
      tooltips: [
        ...
        <p>- you will first <i>break it apart into <b>pieces</b> called components</i>;</p>
        <p>- then you will <i>describe the different <b>visual states</b> for each of your components</i>;</p>
        ...
        <p>1. <i>Break the UI</i> into a <b>component hierarchy</b>;</p>
        <p>2. <i>Build a <b>static version</b></i> in React;</p>
        <p>3. Find the <i>minimal but complete representation of <b>UI state</b></i>;</p>
        ...
        <li>- <i>programming</i>: use the same techniques for deciding if you should create a new function or object. One such technique is the <b>single responsibility principle</b>, that is, <i>a component should ideally only do one thing</i>. If it ends up growing, it should be decomposed into smaller subcomponents.</li>
        <li>- CSS</li>
        <li>- design</li>
        </ul>
        <p>If your JSON is well-structured, you'll often find that it naturally maps to the component structure of your UI. That's because UI and data models often have the same information architecture — that is, the same shape. <i>Separate your UI into components</i>, where each component matches one piece of your data model.</p>
        <p><i>When you finish to idenitfy the components in the mockup, arrange them into a hierarchy. Components that appear within another component in the mockup should appear as a child in the hierarchy.</i></p>
        `,
        ...
        <p>It's often easier to <b>build the static version first and add interactivity later</b>. Building a static version requires a lot of typing and no thinking, but adding interactivity requires a lot of thinking and not a lot of typing.</p>
        ...
        <p>You can either build “top down” by starting with building the components higher up in the hierarchy or “bottom up” by working from components lower down. <i>In simpler examples, it's usually easier to go top-down, and on larger projects, it's easier to go bottom-up.</i></p>
        `,
        ...
        <p><i>Think of state as the minimal set of changing data that your app needs to remember.</i> <b>The most important principle for structuring state is to keep it DRY (Don't Repeat Yourself).</b> <i>Figure out the absolute minimal representation of the state your application needs and <b>compute everything else on-demand</b>.</i></p>
        <ul>Which of the data that your application has are state? Identify the ones that are not:
        <li>- Does it <i>remain unchanged</i> over time? If so, it isn't state.</li>
        <li>- Is it <i>passed in from a parent</i> via props? If so, it isn't state.</li>
        <li>- <i>Can you compute it</i> based on existing state or props in your component? If so, it definitely isn't state!</li>
        <li>What's left is probably state.</li>
        </ul>
        `,
        `<h3>Step 4: Identify where your state should live</h3>
        <p>After identifying your app's minimal state data, you need to <b>identify which component is responsible for <u>changing</u> this state, or <u>owns</u> the state</b>.</p>
    ...
    },
    {
      ...
      tooltips: [
        ...
        <li>- <i>It minds its own business</i>: it does <b>not change any objects or variables that existed before it was called</b>;</li>
        <li>- <i>Same inputs, same output</i>: <b>given the <u>same inputs</u>, a pure function should always return the <u>same result</u></b>.</li>
        </ul>
        <p><i>React assumes that every component you write is a pure function. This means that React components you write <b>must always return the same JSX given the same inputs</b></i>.</p>
        `,
        `<h3>Side Effects: (un)intended consequences</h3>
        <p>React's <b><u>rendering</u> process must always be pure</b>. Components should <i>only return their JSX</i>, and <i>not change any objects or variables that existed <u>before rendering</u></i> — that would make them impure!</p>
        ...
        <p>Pure functions <b>don't mutate variables outside of the function's scope or objects that were created <u>before the call</u></b> — that makes them impure!</p>
        <p>However, <b>it's completely fine to change variables and objects that you've just <u>created while rendering</u></b>.</p>
        `,
        ...
        <p>In React, <b>side effects usually belong inside <u>event handlers</u>. Even though event handlers are defined inside your component, <u>they don't run during rendering</u>!</b> So <i>event handlers don't need to be pure</i>.</p>
    ...
    },
    {
      ...
      tooltips: [
        ...
        <p><b>You can't return more than one "root" JSX element (you also can't store more than one "root" JSX element in a variable)</b>.</p>
        ...
        <p><i>Fragments let you <b>group a list of children without adding extra nodes to the DOM</b>.</i> Fragments allow you to write cleaner code, to end up with less unnecessary HTML elements on the browser.</p>
    ...
    },
    {
      ...
      tooltips: [
        ...
        <p>You can use a Portal to <b>keep the structure that you have in your JSX markup, but to render the JSX markup differently (in other place) in the real DOM</b>.</p>
        ...
        <p><i>A Portal only changes the physical placement of the DOM node.</i> In every other way, <b>the JSX you render into a portal acts as a child node of the React component that renders it</b>. For example, the child can access the context provided by the parent tree, and events still bubble up from children to parents according to the React tree.</p>
        `,
        ...
        <p><b>Even though a portal can be anywhere in the DOM tree, it behaves like a normal React child in every other way.</b> Features like context work exactly the same regardless of whether the child is a portal, as the portal still exists in the <u>React tree</u> regardless of position in the <u>DOM tree</u>.</p>
  ...
  sections: [
    {
      ...
      tooltips: [
        `<p>You will often want to <b>display multiple <u>similar components</u> from a <u>collection of data</u></b>. You can use the JavaScript array methods to <i>manipulate an array of data</i>. You can use JavaScript's <code>filter()</code> and <code>map()</code> with React to <i><u>filter</u> and <u>transform</u> your <b>array of data</b> into an <b>array of components</b></i>.</p>
        <p><b>For each array item, you will need to specify a <code>key</code></b>. Usually, you will want to use an ID from the database as a <code>key</code>. <i>Keys let React keep track of each item's place in the list even if the list changes</i>.</p>
        `,
        `<h3>Render components from an array & render only specific components from an array</h3>
        <p>You will often need to <i>show several <u>instances of the same component</u> using <u>different data</u></i> when building interfaces. You will rely on JavaScript features like <code>for</code> loop, the array <code>map()</code> and <code>filter()</code> functions to <b>render lists of components</b>.</p>
        <p>If you do <i>output an <b>array of JSX elements</b>, then React is capable of rendering on the UI that array of JSX elements</i>. So if you had <b>an array of JSX elements as part of your JSX markup</b>, React would simply render that array of JSX elements side by side.</p>
        `,
        `<h3>Keeping list items in order with <code>key</code></h3>
        <p>You need to give each array item a <code>key</code> — a string or a number that <i><b><u>uniquely</u> identifies</b> that item among other items in that array, in other words among its siblings</i>.</p>
        ...
        <p>What do you do when each item needs to render not one, but several DOM nodes? The short <code><><span><</span>/></code> Fragment syntax won't let you pass a key, so you need to either group them into a single <<span>div</span>>, or use the slightly longer and more explicit <i><code><<span>Fragment key={id}</span>></code></i> syntax.</p>
        `,
        `<h3>Where to get your <code>key</code></h3>
        <p><i>Rather than generating keys on the fly, you should include them in your data.</i></p>
        ...
        <p>Similarly, <i>do not generate keys on the fly, e.g. with <code>key={Math.random()}</code>. This will cause keys to never match up between renders, leading to all your components and DOM being recreated every time.</i></p>
        `,
        `<h3>Rules of keys</h3> 
        <p>- <b>Keys must be <u>unique</u> among siblings</b>. However, it's okay to use the same keys for JSX nodes in different arrays.<p>
        <p>- <b>Keys must <u>not change</u></b> or that defeats their purpose! Don't generate them while rendering.</p>
        `,
    ...
    },
    {
      ...
      tooltips: [
        ...
        <p>Your components will often need to <i><b>display different things</b> depending on <b>different conditions</b></i>. In React, you can conditionally render JSX using JavaScript syntax like <code>if</code> statements, Conditional (ternary) operator <code>? :</code>, and Logical AND operator <code>&&</code>.</p>
        ...
        <p>In some situations, you won't want to render anything at all. <i><b>A component must return something.</b> In this case, you can return <code>null</code>.</i></p>
        <p>In practice, <i>returning <code>null</code> from a component isn't common</i> because it might surprise a developer trying to render it. More often, you would <b>conditionally include or exclude the component in the parent component's JSX</b>.</p>
        `,
        `<h3>How to conditionally include or exclude a piece of JSX</h3>
        <p>When it comes to output content conditionally, <i>long statements like <code>if..else</code> or <code>for</code> loops are not allowed between curly braces, in JSX markup</i>. So, for achive your results, you need to use Ternary Operator <b><code>? :</code></b> or Logical AND operator <b><code>&&</code></b>.</p>
        ...
        <p><i>You can use an <code>if</code> or <code>if..else</code> statement to <b>conditionally assign JSX to a variable and then <u>include</u> it in your returned JSX tree</b>.</i></p>
        <p>NOTE: You can <i>store JSX markup in variables</i>. You are not limited to using JSX only if you return it, you can also use it to create a value, which is stored in a variable. In React, markup is a part of your code, so you can use tools like variables and functions to tidy up complex expressions.</p>
        `,
  ...
  sections: [
    ...
    },
    {
      ...
      tooltips: [
        ...
        <p>State can hold any kind of JavaScript value, including objects. But <b>you shouldn't change objects that you hold in the React state directly</b>. Instead, <i>when you want to update an object, you need to <b>create a new one (or make a copy of an existing one), and then set the state to use that copy</b></i>. Usually, you will use the <code>...</code> spread syntax to copy objects and arrays that you want to change.</p>
        <p>You should <b>treat object in React as if they were immutable (unchangeable or "read-only")</b>, so <b>treat any JavaScript object that you put into state as read-only</b>.</p>
        <p><b>When you <u>store objects in state</u>, <u>mutating them will not trigger renders</u> and <u>will change the state in previous render "snapshots"</u></b>. Instead of mutating an object, create a new version of it, and trigger a re-render by setting state to it.</p>
        <p><b>In React, you treat all state as immutable!</b></p>
        `,
    ...
    },
    {
      ...
      tooltips: [
        `<h3>Treat state as read-only</h3>
        <p>Arrays are mutable in JavaScript, but <i>you should <b>treat arrays as immutable when you store them in state</b></i>. Just like with objects, when you want to update an array stored in state, you need to <i>create a new one (or make a copy of an existing one), and then set state to use the new array</i>.</p>
        <p>You can put arrays into state, but you can't change them.</p>
        `,
        `<h3>Updating arrays without mutation</h3>
        <p>In JavaScript, arrays are just another kind of object. Like with objects, <i>you should <b>treat arrays in React state as read-only</b></i>. This means that you <i>shouldn't reassign items inside an array</i> like <code>arr[0] = 'bird'</code>, and you also <i>shouldn't use methods that mutate the array</i>, such as <code>push()</code> and <code>pop()</code>.</p>
        ...
        <p><b>Objects are not really located "inside" arrays. They might appear to be "inside" in code, but each object in an array is a separate value, to which the array "points"</b>.</p>
  ...
  sections: [
    ...
    },
    {
      ...
      tooltips: [
        ...
        <p><i>If you can <b>calculate some information from the component's props or its existing state variables during rendering</b>, you should not put that information into that component's state.</i></p>
        <p><b>Don't mirror props in state!</b> If you initialize a state variable with a prop, if the parent component passes a different value for that prop, that state variable would not be updated! That's because <i>the state is only initialized during the <u>first render</u></i>. "Mirroring" props into state only makes sense when you want to <i>ignore all updates for a specific prop</i>. By convention, start the prop name with "initial" or "default" to clarify that its new values are ignored.</p>
        `,
    ...
    },
    {
      ...
      tooltips: [
        ...
        <p><i>When you render a <u>different component</u> in the <u>same position</u>, it <b>resets the state of its <u>entire subtree</u></b>.</i></p>
        ...
        <p>- <i>You could render all components instead of just the current one, but <b>hide all the others with CSS</b>.</i> The components would not get removed from the tree, so their local state would be preserved. This solution works great for simple UIs. But it can get very slow if the hidden trees are large and contain a lot of DOM nodes.</p>
        <p>- <i>You could <b>lift the state up</b> and hold the state of child components in the parent component.</i> This way, when the child components get removed, it doesn't matter, because it's the parent that keeps the important information.</p>
        <p>- <i>You might also <b>use a different source in addition to React state</b>.</i></p>
        `,
      ],
    },
    {
  ...
  sections: [
    {
      ...
      tooltips: [
        ...
        <p>There's one important note. You also saw this ReactDOM thing and in the end <i><b>ReactDOM is your interface to the web</b>. <b>ReactJS itself does not know the web</b>, it does know nothing about the browser in the end. ReactJS knows how to work with components but it doesn't care whether those components contain HTML elements or if they contain totally fictional elements</i>, that does not matter to ReactJS. It's ReactDOM to which that matters in the end and which ultimately needs to bring real HTML elements to the screen.</p>
        ...
        <p>React is concerned about components and what React in the end does is, it uses a concept called the <i>virtual DOM</i>. With the help of virtual DOM, <i>React determines how the component tree currently looks like and what it should look like, for example, after a state update</i>. And that information is then handed off to ReactDOM which now knows about the differences and which then knows how it should <b>manipulate the real DOM to match that virtual DOM</b>, that virtual snapshot React derived for your component trees.</p>
        <p>Whenever state, props or a context of a component changes, that component function is re-executed, that component is re-evaluated by React. But it is worth noting that <i><b>re-evaluating a component is not the same as re-rendering the DOM</b>. Just because a component function is re-executed by React does not mean that the respective part of the actual real DOM is re-rendered or re-evaluated. Instead you have to differentiate between your component part (your React part) and the real DOM.</i></p>
    ...
    },
    {
      ...
      tooltips: [
        `<p>A component that is <i>passed inside another component as a <code>children</code> prop</i> re-render slightly different than a component that is <i>passed directly inside another component</i>. Why? Because <b>a component that is passed via <code>children</code> is already been created before anything happens inside the parent component</b> (like state updates in parent component). The state updates in the parent component will re-render the parent component, but will NOT RE-RENDER the child components passed via <code>children</code>.</p>
    ...
    },
    {
      ...
      tooltips: [
        ...
        <p><b>React normally re-renders a component whenever its parent re-renders.</b> <i>With <code>React.memo()</code> you can create a component that React will not re-render when its parent re-renders so long as its new props are the same as the old props.</i> Such a component is said to be <i>memoized</i>.</p>
        <p><i><code>React.memo()</code> lets you <b>skip re-rendering a component when its props are unchanged</b></i>. It help you optimize functional components to <i>avoids unnecessary components re-rendering</i>.</p>
        <p><i><code>React.memo()</code> only work for functional components.</i></p>
        <p><b>If you wrap a component with <code>React.memo()</code>, if that wrapped component is not re-evaluated, its children components are also not re-evaluated.</b></p>
        <p>A React component should always have pure rendering logic. This means that it must return the same output if its props, state, and context haven't changed. By using <code>React.memo()</code>, you are telling React that your component complies with this requirement, so React doesn't need to re-render as long as its props haven't changed. <b>When you use <code>React.memo()</code>, your component will still re-render <u>if its own state changes</u> or <u>if a context that it's using changes</u>.</b> <i>Memoization only has to do with props that are passed to the component from its parent.</i></p>
        `,
        ...
        <p>Keep in mind that <b><code>React.memo()</code> is completely useless if the props passed to your component are <u>always different</u></b>, <i>such as if you pass an object or a plain function defined during rendering. This is why you will often need <code>useMemo</code> and <code>useCallback</code> together with <code>React.memo()</code></i>.</p>
        `,
        `<h3>When to use <code>React.memo()</code>?</h3>
        <p><code>React.memo()</code> can be a great tool if you have a huge component tree with a lot of child components, and on a high level in the component tree <i>you can avoid unnecessary re-render cycles for the entire branch of the component tree</i>, because even children components of the parent component that is wrapped by <code>React.memo()</code> will not be re-executed. <b>With the help of <code>React.memo()</code> you can cut off entire branches of unnecessary re-evaluations.</b></p>
    ...
    },
    {
      ...
      tooltips: [
        ...
        <p>When you optimize rendering performance, you will sometimes need to <b>cache the functions that you pass to child components</b>. <i>To cache a function between re-renders of your component, wrap its definition into the <code>useCallback</code> hook.</i></p>`,
    ...
    },
    {
      ...
      tooltips: [
        ...
        <p><code>useMemo</code> lets you <b>cache the result of a calculation between re-renders until its dependencies change</b>.</p>`,
  ...
  sections: [
    {
      ...
      tooltips: [
        ...
        <p>Custom hooks are just <i>regular functions</i>, just as the built-in hooks like <code>useState</code>, but they are functions which can <b>contain stateful logic</b>. You can build custom hooks to <b>outsource stateful logic into <u>reusable</u> functions</b>.</p>
        <p>Unlike regular functions, <b>custom hooks can use other React hooks, including other custom hooks</b>, and they can, therefore, also leverage React state managed with <code>useState</code> or <code>useReducer</code>; they can access <code>useEffect</code> and so on.</p>
        ...
        <p><i>Custom Hook names always <b>start with "use" followed by a capital letter</b>.</i></p>
        ...
        <p>Custom Hooks let you <b>share stateful logic but <u>not state itself</u>. Each call to a Hook is completely independent from every other call to the same Hook</b>.</p>
        ...
        <p><b>The code inside your custom Hooks will re-run during every re-render of your component</b>. This is why, like components, custom Hooks need to be pure. <i>Think of custom Hooks code as part of your component's body!</i></p>
        <p><i>Because custom Hooks re-render together with your component, they always receive the latest props and state.</i></p>
        ...
        <p>No. <i>Functions that don't call Hooks don't need to be Hooks</i>. <b>If your function doesn't call any Hooks, avoid the "use" prefix</b>. <i>Instead, write it as a regular function without the "use" prefix. This ensures that your code can call that regular function anywhere, including conditions.</i></p>
...
const React_Router = {
  ...
  sections: [
    ...
    },
    {
      ...
      tooltips: [
        `<p><b>On each route you can define a "loader" function to <u>provide data</u> to the route element <u>before it renders</u></b>. <i>The route "loader" function is <b>called before the route component is render</b> and provides data for the component through <code>useLoaderData</code>.</i></p>
        ...
        <p>With the help of a "loader" function you <b>move all the fetch logic that you have in a component in that "loader" function</b>, and this process make the component way leaner and easier to reason about.</p>
        `,
        ...
        <p><i><code>useLoaderData</code> <b>always get the final data returned by the "loader" function</b>, even if that "loader" function is an async function.</i> If the "loader" function is an async function, the <code>useLoaderData</code> won't return a Promise from the "loader" function, but will return the resolved/rejected value of that async "loader" function.</p>
        <p><i>The <code>useLoaderData</code> is a hook which you can execute to <b>get access to the closest "loader" function data</b>.</i> So you can access "loader" function data with the help of <code>useLoaderData</code> in <i>any component on the <u>same level or lower level</u> than the component where you added the "loader" function</i>, so the route on which you added the "loader" function.</p>
        <p><i>You can use <code>useLoaderData</code> in the element that's assigned to a route and in all components that might be used inside that element</i>.</p>
        <p>You can use this hook in any component or any custom hook, not just the Route element. It will return the data from the nearest route on context. <b>To get data from any <u>active route</u> on the page, see <code>useRouteLoaderData</code>.</b></p>
        `,
        ...
        <p>One important aspect of a "loader" function is to understand that <i><b>you can return any kind of data</b> in that "loader" function</i>. You can return an array, object, number, string etc, but what you can also return is a <code>Response</code> object.</p>
    ...
    },
    {
      ...
      tooltips: [
        `<ul>Within a "loader" function you can <i>get access to the route parameters</i> because when the React Router will call the "loader" function passes to it an <u>object parameter</u> that contains two important pieces of data which contains a <b><code>request</code> object</b> and a <b><code>params</code> object</b>:
    ...
    },
    {
      ...
      tooltips: [
        ...
        <p>You can use the <i><u>nested routes</u></i> feature not just as a wrapper layout component, but also to <i><b>share a "loader" function</b> from parent to child routes</i>. Because as you learned, <i>you can access "loader" function data in any component that's on the same level or a lower level than the route where the "loader" function is added</i>.</p>
        <p><i>Instead of defining the same "loader" function twice for each sibling route, you can add a route parent that does not render an element.</i> Now in that parent route you don't need an element because you don't want to have any shared layout or anything like that. Instead, you will using this approach because you want to <i>add a "loader" function to the parent route so its child routes can have access to the returned data of its "loader" function</i>. This is also how you can use nested routes to construct a URL, with the parent route URL and the child routes URLs.</p>
        `,
        `<h3><code>useLoaderData</code> vs <code>useRouteLoaderData</code></h3>
        <p>To access the data returned by the shared "loader" function, instead of using <code>useLoaderData</code> you need to use a different hook which is called <code>useRouteLoaderData</code>. <i>This hook works almost like <code>useLoaderData</code> but it takes a <b>route ID as an argument</b> (the ID you assign to the parent route).</i></p>
        <p>The <code>useLoaderData</code> doesn't work in this case because this hook <i>provides the value returned from your route "loader" function, but you want the get the data from the parent route</i>.</p>
        <p>With the <code>useRouteLoaderData</code> you can <i><b>get access to the data of a higher level "loader" function from a route that doesn't have a "loader" function</b>. Now you can <b>reuse the same "loader" function across <u>multiple routes which all need the same data</u></b></i>.</p>
        ...
        <p><i>The only data available is the routes that are <u>currently rendered</u></i>. <b>If you ask for data from a route that is not currently rendered, the hook will return <code>undefined</code>.</b></p>
        `,
      ],
    },
    {
      ...
      tooltips: [
        `<p>In your "loader" function you can <code>throw</code> an error. This will break out of the current call stack (stop running the current code) and React Router will start over down the "error path". <b>When you <code>throw</code> in a "loader" function something special happens: React Router will simply render the closest <code>errorElement</code></b>.</p>
        ...
        <p><i>To differentiate between different errors like <code>404</code> or <code>500</code>, you can <code>throw</code> in your "loader" function a <code>Response</code> object and you can include some data into that <code>Response</code>.</i> You thrown an <code>Response</code> object with some data attached to it because <i>you can actually get hold of the data that's being thrown as an error inside of the component that's being rendered as an <code>errorElement</code></i> and for that <code>react-router-dom</code> gives you a hook called <code>useRouteError</code>. <i>Inside of an <code>errorElement</code>, this hook returns anything thrown during an "loader"/"action" function or rendering</i>.</p>
        <p><i>The <code>useRouteError</code> return a object, and the returned object depends on whether you threw a <code>Response</code> or any other kind of object or data.</i></p>
    ...
    },
    {
      ...
      tooltips: [
        `<p>React Router gives us a <code>useNavigation</code> hook which we can use to <i><b>check the current route transitions state</b>, so to find out if a transition has been initiated and we're currently still waiting for data to arrive, or if we're done</i>.</p>
        <p><i><code>useNavigation</code> let you show/dispay a feedback when a user navigate to a page.</i></p>
        ...
        <p>It's important to undestand that <i><b><code>useNavigation</code> won't be called on the page which you're transitioning to</b>, but instead on some page or a component which is already visible on the screen when the transition is started</i>.</p>
        `,
      ],
    },
    {
      ...
      tooltips: [
        `<p>Just as you can add <b>"loader" functions to <u>load</u> data</b>, you can also add <b>"action" functions to <u>send</u> data</b>. <i>Like the "loader" property, "action" property wants a function as a value.</i></p>
        ...
        <p><b>When the "action" function completes, all of the "loader" function data on the page will automatically revalidate to keep your UI in sync with your data.</b></p>
        `,
    ...
    },
    {
      ...
      tooltips: [
        ...
        <p><i>The typical and <u>default way</u> of triggering an "action" function is by using the <<span>Form</span>> component.</i> The <<span>Form</span>> component will <i><u>automatically</u> trigger the "action" function of the currently active route</i>, so the route for which the <<span>Form</span>> component was loaded.</p>
        <p>Just as you can navigate programmatically, you can also <b>submit data and trigger an "action" function <u>programmatically</u> with the <code>useSubmit</code> hook</b> imported from <code>react-router-dom</code>.</p>
  ...
  sections: [
    {
      ...
      tooltips: [
        `<ul>In the intricate dance of web application development, <i>the <b>management of data</b> falls into two primary categories:</i>
        <li>- <b><u>Client</u> State</b></li>
        <li>- <b><u>Server</u> State</b></li>
        </ul>`,
        ...
        <p>- Server State: On the other hand, server state refers to <i>data that is owned and managed by the server</i>. Examples include lists of articles, user details, or <i>any information retrieved from the server</i>.</p>`,
        ...
        <p>- Server State: <i>React Query introduces a different approach. With its <b>caching mechanism</b>, you can use the same <code>useQuery</code> hook in multiple components, and it will fetch the data once and subsequently return it from the cache.</i> This reduces the need for managing client state globally, as <b>the server state is effectively cached</b> and accessible as needed.</p>
        `,
        ...
        <p><i>In few words, <b>global UI state is state that IS NOT FETCHED from a server</b>. You only need Redux if you want to manage a lot of non-remote global state that updates frequently.</i> But the truth is that most global state is actually remote, which is the reason why so many apps don't use Redux anymore.</p>
        `,
      ],
    },
    {
      ...
      tooltips: [
        ...
        <p><i>The library operates on <b>sane defaults</b> that try to <b>keep your data as fresh as possible</b> while at the same time <b>showing data to the user as early as possible</b></i>, making it feel near instant at times and thus providing a great UX.</p>
        <p>IMPORTANT: <i>React Query is in fact NOT a data fetching library. It doesn't fetch any data for you. React Query is an <b>async (server) state manager</b>.</i> It can manage any form of asynchronous state - it is happy as long as it gets a Promise.</p>
        `,
        `<h3>Data is stored in a cache</h3>
        <p>The most fundamental thing about React Query is that <i><b>all remote state is cached</b>, which means that <b>the fetched data will be stored in order to be reused</b> in different points of the application.</i></p>
        ...
        <p><i>We can say that React Query is a <b>data synchronization tool</b> because <b>automatically refetches the data in certain situations</b></i>. For example, after a certain timeout or after we leave the browser window and then come back to it.</p>
        ...
        `,
        `<h3>Prefetching</h3>
        <p><i>Besides refetching, we can also prefetch data, which basically means to <b>fetch data that we know will become necessary later, but before it is actually displayed on the screen</b>.</i></p>
        `,
        `<h3>Offline support</h3>
        <p>A feature that I find really useful is <i>support for when the user becomes offline</i>. So, in this situation, since the <i><u>data is already cached</u></i>, as the user moves around in the app while being offline, Components A and B can still be displayed using the cached data.</p>
        `,
        ...
        <p>We need a library like React Query with all these features because <i><b><u>remote state</u> is fundamentally different from <u>UI state</u></b>. Remote state is asynchronous and usually shared by many users of the app, which makes it so that applications running in different browsers can very easily get out of sync with the remote data that is stored on a server</i>.</p>
        ...
        <p>For a vast majority of applications, <i>the truly <u>globally accessible client state</u> that is left over after migrating all of your async code to TanStack Query is usually very tiny</i>.</p>
        `,
      ],
    },
    {
      ...
      tooltips: [
        ...
        <p>We install React Query by typing in our VSCode terminal the <i><code>npm install <b>@tanstack/react-query</b></code></i> command. The library itself is actually called <i>TanStack Query</i> because it also works in other frameworks such as Svelte or Vue.</p>
        `,
        `<h3>Set up the <u>React Query cache</u> & <u>provide it</u> to the app</h3>
        <p><i>The idea behind integrating React Query into our application is very similar to what we did with the Context API or Redux. So is a similar idea of <b>having the data in one place</b>, and then <b>providing it to the whole component tree</b>.</i></p>
        <ul>To integrating React Query into our application, we need to:
          <li>- <i>set up the <b>React Query cache</b> (create a place where the data lives) by using <b><code>new QueryClient()</code></b>;</i></li>
          <li>- <i><b>provide that cache</b> (data) to our application by using the <b><code><<span>QueryClientProvider</span>></code></b> component.</i></li>
        </ul>
        <p>IMPORTANT: <i>Place the <code><<span>QueryClientProvider client={queryClient}</span>></code> as high in your React app as you can. The closer it is to the root of the page, the better it will work!</i></p>
    ...
    },
    {
      ...
      tooltips: [
        ...
        <p>We can install the React Query DevTools. Just like Redux, React Query also has some excellent DevTools, but we just set them up in a different way. <b>In the case of React Query DevTools, it is simply an <u>NPM package</u> and not something in the browser.</b></p>
        <p>We install the React Query DevTools by typing into the VSCode terminal the <i><code>npm install <b>@tanstack/react-query-devtools</b></code></i> command. <i>After we install the NPM package, we need to <b><code>import { ReactQueryDevtools } from '@tanstack/react-query-devtools'</code></b> and wrapping the entire app with it.</i></p>
        `,
        `<h3>React Query DevTools: <code>stale</code>, <code>inactive</code> & <code>fresh</code> status</h3>
        <p>The <code>stale</code> status that you see in React Query DevTools means that the <i>data is out of date (old or stale)</i>. So therefore, <i>when we do certain things, React Query will automatically refetch the data</i>. One of the things that we can do, which will then trigger a refetch, is to move away from the current browser tab and then come back to it later. So that will trigger a refetch as soon as the data is stale (<code>stale</code> status).</p>
        <p>IMPORTANT: <i><b>As long as data has <code>fresh</code> status, it will always come from the cache only.</b> You will not see a network request for fresh data, no matter how often you want to retrieve it.</i> <b>You can control the <code>stale</code> status time by overwrite the <code>staleTime</code> property.</b></p>
        <p><i><b>The data in the cache becomes inactive (has the <code>inactive</code> status) when the <u>component that holds that data is unmounted from the DOM</u>.</b> Although the component is unmounted from the DOM, the data that correspond to that component is preserved in the React Query cache.</i> When the component is remounted to the DOM, React Query will refetch the data that correspond to that component automatically, but only if it has the <code>stale</code> status. Keep in mind that <i>as long as a component has the <code>fresh</code> status, the data in the cache that correspond to that component will not be refetch again, even if the component is unmounted and remounted to the DOM.</i></p>
    ...
    },
    {
      ...
      tooltips: [
        ...
        <li>1. <i>The <b><code>queryKey</code></b> that <b>uniquely identify the data</b> that we're going to query.</i> <b>The <code>queryKey</code> needs to be <u>an array</u>.</b> The unique key you provide is used internally for caching, refetching, and sharing your queries throughout your application.</li>
        <li>2. <i>The <b><code>queryFn</code></b> (query function). <code>queryFn</code> is the function that the query will use to <b>request data from the API</b>.</i> What's important is that <b>the query function needs to return a Promise that resolves the data or throws an error</b>.</li>
        </ul>`,
        `<h3><code>useQuery</code> returns an object</h3>
        <p>The <code>useQuery</code> hook will <b>return an object</b> with a bunch of useful properties that we can use in our app. The most important property is <i><code>data</code></i>. Another useful properties are: <i><code>isLoading</code></i> and <i><code>status</code></i>.</p>
        ...
        <p><i>At its core, TanStack Query <b>manages query caching for you based on query keys (<code>queryKey</code>)</b>.</i></p>
        <p>Query Keys are a very important core concept in React Query. They are necessary so that the library can internally <i>cache your data correctly</i> and <b>refetch automatically when a dependency to your query changes</b>. Lastly, it will allow you to <i>interact with the Query Cache manually when needed</i>, for example, when <u>updating data after a mutation</u> or when <u>you need to manually invalidate some queries</u>.</p>
        ...
        <p>Note that <b>query keys act as dependencies for your query functions</b>. <i>Adding dependent variables to your query key will ensure that queries are cached independently, and that any time a variable changes, queries will be refetched automatically (depending on your <code>staleTime</code> settings).</i></p>
        `,
        ...
        <p>Why are these two similar? Because <b>React Query will trigger a refetch whenever the query key changes</b>. So when we pass a variable parameter to our <code>queryFn</code>, we almost always want to fetch data when that value changes.</p>`,
        `<h3>Query Functions (<code>queryFn</code>)</h3>
        <p>A query function can be literally <b>any function that <u>returns a Promise</u></b>. The Promise that is returned should either <u>resolve the data</u> or <u>throw an error</u>.</p>
    ...
    },
    {
      ...
      tooltips: [
        `<p><b>Cache invalidation</b> is pretty hard, so when do you decide it's time to ask the backend again for new data? Surely <i>we can't just do this every time a component that calls <code>useQuery</code> re-renders</i>. That would be insanely expensive, even by modern standards.</p>`,
        `<h3>Smart refetches</h3>
        <ul>React Query is smart and chooses <i>strategic points for triggering a refetch</i>. These are:
          <li>- <b><code>refetchOnMount</code></b>: whenever a new component that calls <code>useQuery</code> mounts, React Query will do a revalidation.</li>
          <li>- <b><code>refetchOnWindowFocus</code></b>: whenever you focus the browser tab, there will be a refetch.</li>
          <li>- <b><code>refetchOnReconnect</code></b>: if you lose your network connection and regain it, it's also a good indicator to revalidate what you see on the screen.</li>
        </ul>
        <p>Finally, if you, as the developer of your app, know a good point in time, <i>you can invoke a <u>manual invalidation</u> via <b><code>queryClient.invalidateQueries</code></b></i>. This comes in very handy after you perform a mutation.</p>
        <p>IMPORTANT: <b>As long as data has <code>fresh</code> status, it will always come from the cache only.</b> <i>You will not see a network request for fresh data, no matter how often you want to retrieve it.</i> You can control the <code>stale</code> status time by overwrite the <code>staleTime</code> property. On the other hand, <i><b><code>queryClient.invalidateQueries</code> mark query/queries as stale</b>. This stale state overrides any <code>staleTime</code> configurations being used in <code>useQuery</code></i> or related hooks.</p>
        `,
        `<h3>React Query defaults: <code>staleTime</code></h3>
        <p>React Query comes with aggressive but sane defaults, but they are geared towards keeping things up-to-date, not to minimize the amount of network requests. This is mainly because <i><code>staleTime</code> defaults to zero, which means that every time you mount a new component instance, you will get a background refetch</i>.</p>
        <p><i>Query instances via <code>useQuery</code> by default consider cached data as stale. To change this behavior, you can <b>configure your queries both <u>globally</u> and <u>per-query</u> using the <code>staleTime</code> option</b>. Specifying a longer <code>staleTime</code> means queries will not refetch their data as often.</i></p>
        <ul><i><u>Stale</u> queries are refetched automatically in the background when</i>:
          ...
          <li>- <i>The query is optionally configured with a refetch interval;</i></li>
          <p>To change this functionality, you can use options like <code>refetchOnMount</code>, <code>refetchOnWindowFocus</code>, <code>refetchOnReconnect</code> and <i><code>refetchInterval</code></i>.</p>
        </ul>
        <p>Customize <code>staleTime</code>: set <code>staleTime</code> to a value you are comfortable with for your specific use-case. The key thing to know is: As long as data is fresh, it will always come from the cache only. You will not see a network request for fresh data, no matter how often you want to retrieve it. There is also no "correct" value for <code>staleTime</code>.</p>
        `,
        `<h3>React Query defaults: garbage collection & <code>gcTime</code></h3>
        <p>Query results that have no more <i>active instances</i> of <code>useQuery</code> are labeled as "inactive" and remain in the cache in case they are used again at a later time. <b>By default, <u>"inactive" queries</u> are <u>garbage collected</u> after 5 minutes.</b> <i>To change this, you can alter the default <b><code>gcTime</code></b> for queries to something other than "1000 * 60 * 5 milliseconds".</i></p>`,
        `<h3>React Query defaults: query retry</h3>
        <p><i><b><u>Queries that fail</u> are silently retried 3 times, with exponential backoff delay</b> before capturing and displaying an error to the UI.</i> To change this, you can alter the default <b><code>retry</code></b> and <b><code>retryDelay</code></b> options for queries to something other than 3 and the default exponential backoff function.</p>
        `,
        `<h3>Using <code>setQueryDefaults</code></h3>
        <p>React Query supports a great way of <b>setting default values per Query Key via <code>QueryClient.setQueryDefaults</code></b>.</p>
        `,
      ],
    },
    {
      sectionTitle:
        'Disabling/pausing queries (lazy queries) with <code>enable</code> option & <code>refetch</code> function',
      sectionSource:
        '/src/markdowns/16_React_Query/Disabling_pausing_queries_and_lazy_queries.html',
      highlights: {
        highlight1: ['lazy queries'],
        highlight2: ['enable', 'refetch'],
      },
      tooltips: [
        `<h3>Disabling/Pausing Queries</h3>
        <p>If you ever want to <b>disable a query from <u>automatically running</u></b>, you can use the <b><code>enabled = false</code></b> option.</p>
        <ul>When <code>enabled</code> is false:
         ...
         <li><b>- The query will not automatically fetch on mount.</b></li>
         <li><i>- The query will not automatically refetch in the background.</i></li>
         <li><i>- <b>The query will ignore query client <code>invalidateQueries</code> and <code>refetchQueries</code> calls</b> that would normally result in the query refetching.</i></li>
         <li><i>- <b><code>refetch</code> function</b> returned from <code>useQuery</code> can be used to <b>manually trigger the query to fetch</b>.</i></li>
        </ul>
        <p>Permanently disabling a query opts out of many great features that TanStack Query has to offer (like background refetches), and it's also not the idiomatic way. It takes you from the declarative approach (defining dependencies when your query should run) into an imperative mode (fetch whenever I click here). It is also <b>not possible to pass parameters to <code>refetch</code></b>.</p>
        `,
        `<h3>The <code>refetch</code> function, query keys and automatic refetching</h3>
    ...
    },
    {
      ...
      highlights: {
        highlight1: ['fetch data programatically'],
      },
      tooltips: [
    ...
    },
    {
      ...
      tooltips: [
        ...
        <p><i>When a <code>useQuery</code> query fails (<b>the query function throws an error</b>), TanStack Query will <b>automatically retry the query</b> if that query's request has not reached the max number of consecutive retries (defaults to <code>3</code>) or a function is provided to determine if a retry is allowed.</i></p>
        <ul><i>You can configure retries both on a <u>global level</u> and an <u>individual query level</u>.</i>
          ...
          <li>- Setting <code>retry = true</code> will infinitely retry <u>failing requests</u>.</li>
          <li>- Setting <code>retry = (failureCount, error) => ...</code> allows for custom logic based on why the request failed.</li>
        </ul>
        `,
        `<h3>Retry Delay</h3>
        <p><b>By default, retries in TanStack Query do not happen immediately after a request fails.</b> <i>As is standard, a back-off delay is gradually applied to each retry attempt.</i></p>
    ...
    },
    {
      ...
      highlights: {
        highlight1: ['Mutating data'],
        highlight2: ['<code>useMutation()</code>'],
      },
      tooltips: [
        `<p>Up until now we learned how to fetch and store data in the cache, using the <code>useQuery</code> hook. Now it's time to learn how to also <i>mutate our remote server data, and automatically re-render the UI</i>.</p>
        ...
        <li>1. <i>The <b><code>mutationFn</code></b> method. This is the function that React Query will call to mutate our remote server data.</i></li>
        </ul>`,
        `<h3><code>useMutation()</code> returns an object</h3>
        <p>The <code>useMutation()</code> hook will <b>return an object</b> with a bunch of useful properties and methods that we can use in our app to mutate data on the server. <i>The most important one is the <b><code>mutate</code> function</b>. This function can be connected to our event handlers, inside our React components. <b>The <code>mutate</code> function will call the function that we pass as value to the <code>mutationFn</code> key.</b></i></p>
        `,
        `<h3>The <code>mutationFn</code> function receives only a single parameter</h3>
        <p>You can pass variables to your <code>mutationFn</code> function by <i>calling the <code>mutate</code> function with a <u>single variable or object</u></i>. <b>The <code>mutationFn</code> receives only a single parameter</b>, <i>so if you want to pass to it multiple values, you need to wrap them into an object when you pass them to the <code>mutate</code> function</i>.</p>
        `,
        ...
        <p><i>By defining only the <code>mutationFn</code> property, <b>the UI will not be automatically updated after we mutate the data on our server</b>. If we want to trigger automatically UI updates after we mutate the data that's on our server, to <code>useMutation()</code> we can pass another property that is optional, and is called <b><code>onSucces</code></b>. <code>onSucces</code> will hold a method that will <b>invalidating the cache as soon as the mutation is done</b>, so with <code>onSucces</code> method we tell React Query what to do as soon as the mutation was successful.</i></p>
        <p><i>We <b>manually invalidating the cache</b> by using the <b><code>queryClient.invalidateQueries()</code></b> method.</i> This method tells React Query to immediately refetch the data that's on our server.</p>
        ...
        <p>Another optional property that we can define in the object passed to <code>useMutation()</code> is the <b><code>onError</code> property</b>. <i>The <code>onError</code> property will hold a function that will fire if the mutation encounters an error and will be passed the error.</i> If a Promise is returned, it will be awaited and resolved before proceeding.</p>
        `,
        `<h3>Mutation function options. Specify individual behavior for each <code>mutate</code> function</h3>
        <p><i>You might find that you want to <u>trigger additional callbacks</u> beyond the ones defined on <code>useMutation()</code> when calling <code>mutate()</code>.</i> This can be used to trigger <i>component-specific side effects</i>. To do that, <b>you can also provide to the <code>mutate()</code> function (after your mutation variable) any of the same callback options that you pass to <code>mutationFn</code></b>. <i>Supported options include: <code>onSuccess</code>, <code>onError</code> and <code>onSettled</code>.</i></p>
        <p>IMPORTANT <b>Please keep in mind that those additional callbacks won't run if your component <u>unmounts before the mutation finishes</u>.</b></p>
        ...
        `,
        `<h3>Invalidations from Mutations</h3>
    ...
    },
    {
      ...
      tooltips: [
        ...
        <ul>When a query is invalidated with <code>invalidateQueries</code>, two things happen:
          <li><b>- It is <u>marked as stale</u>. This stale state overrides any <code>staleTime</code> configurations being used in <code>useQuery</code> or related hooks</b>;</li>
        ...
        <ul><i>The <code>invalidateQueries</code> method can be <b>used to <u>invalidate and refetch</u> <u>single</u> or <u>multiple</u> queries in the cache based on their query keys</b> or any other functionally accessible property/state of the query</i>. By default, all matching queries are immediately marked as invalid and active queries are refetched in the background:
    ...
    },
    {
      ...
      highlights: {
        highlight1: ['Manually set data into the React Query cache'],
        highlight2: ['<code>setQueryData</code>'],
      },
      tooltips: [
        `<p>You can <b>manually set data into the React Query cache</b> via <b><code>queryClient.setQueryData</code></b> function. The <code>QueryClient</code> can be used to <i>interact with a React Query cache</i>.</p>
        <p><b><code>setQueryData</code> is a <u>synchronous function</u> that can be used to <u>immediately update a query's cached data</u></b>. <i>If the query does not exist, it will be created.</i></p>
        ...
        <p>The difference between using <code>setQueryData</code> and <code>fetchQuery</code> is that <b><code>setQueryData</code> is synchronous</b> and <b>assumes that you already synchronously have the data available</b>. If you need to fetch the data asynchronously, it's suggested that you either refetch the query key or use <code>fetchQuery</code> to handle the asynchronous fetch.</p>
        `,
        `<h3>Don't use the queryCache as a local state manager</h3>
        <p><i>If you tamper with the queryCache (<code>queryClient.setQueryData</code>), it should only be for optimistic updates or for writing data that you receive from the backend after a mutation. Remember that every background refetch might override that data, so use something else for local state (Redux, Zustand etc.).</i></p>`,
      ],
    },
    {
      sectionTitle:
      ...
      highlights: {
        highlight1: ['Manually remove data from React Query cache'],
        highlight2: ['<code>removeQueries</code>'],
      },
      tooltips: [
        `<p>You can <b>manually remove data from React Query cache</b> with <b><code>queryClient.removeQueries</code></b> function. The <code>QueryClient</code> can be used to <i>interact with a React Query cache</i>.</p>
        <p>The <code>removeQueries</code> method can be used to <i><b>remove queries from the cache based on their query keys</b> or any other functionally accessible property/state of the query</i>.</p>
        <p>IMPORTANT: <b>By writing <code>queryClient.removeQueries()</code> without any parameter, you will remove all queries that have been accumulated in the cache.</b></p>
        `,
      ],
    },
    {
      ...
      tooltips: [
        ...
        <p><b>The way <code>queryClient.prefetchQuery()</code> works is basically exactly the same as the <code>useQuery</code> hook itself, so we need a <code>queryKey</code> and a <code>queryFn</code> inside an object.</b></p>
  ...
  sections: [
    {
      ...
      tooltips: [
        ...
        <li>- <i>Compound Component Pattern</i>: with compound component, we will have multiple components that play together in order to create one big, let's call it super component, which is then the compound component. This pattern allows us to build extremely <u>self-contained components</u> that need to, or that want to <u>manage their own state internally</u>, so without that state being necessary inside the parent component that uses the compound component.</li>
        </ul>
        `,
      ],
    },
    ...
    },
    {
      ...
      tooltips: [
        `<p>Compound Components is a <u>design pattern</u> used in React where <i>you have a component that doesn't render any UI itself, but instead serves as a container for other components. These "child" components are typically tightly related and work together to create a unified UI.</i></p>
